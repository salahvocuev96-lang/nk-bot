...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users (user_id BIGINT PRIMARY KEY, first_name TEXT, username TEXT, group_name TEXT, full_name TEXT DEFAULT NULL, phone TEXT DEFAULT NULL, is_verified INTEGER DEFAULT 0, last_active TEXT DEFAULT NULL)''')
...
def get_day_name(day_num=None):
    ...
    if day_num is None: day_num = datetime.datetime.now(TIMEZONE).weekday()
    return days[day_num]
...
def main_menu_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("🆘 Помощь", callback_data='help')] # Кнопка Практика удалена
    ]
...
async def admin_command(update: Update, context):
    if update.effective_user.id != ADMIN_ID: return await update.message.reply_text("⛔ Доступ запрещен!")
...
async def active_users_command(update: Update, context):
    ...
    for uid, fname, full, grp, last in users: text += f"🆔 `{uid}` | {full or fname} | {grp}\n   🕐 Последняя активность: {last}\n\n"
    await update.message.reply_text(text, parse_mode='Markdown')
...
async def upload_schedule_command(update: Update, context):
    ...
    if not doc and update.message.reply_to_message: doc = update.message.reply_to_message.document
    if not doc: return await update.message.reply_text("️ Нужно прикрепить файл к команде или ответить на файл командой /upload_schedule.\n\nФайл должен быть в формате CSV со столбцами: Группа, День, Время, Предмет, Преподаватель, Аудитория")
            ...
            for row in reader:
                ...
                try:
                    ...
                    if group and day and time and subject:
                        c.execute('''INSERT INTO schedule (group_name, day, time, subject, teacher, room) VALUES (%s, %s, %s, %s, %s, %s)''', (group, day, time, subject, teacher, room))
                        count += 1
...
async def add_schedule_command(update: Update, context):
    if update.effective_user.id != ADMIN_ID: return await update.message.reply_text("⛔ Только для админа!")
    if len(context.args) == 5: group_name, day_short, time, subject, teacher, room = "ОБЩЕЕ", context.args[0], context.args[1], context.args[2], context.args[3], context.args[4]
    elif len(context.args) == 6: group_name, day_short, time, subject, teacher, room = context.args[0], context.args[1], context.args[2], context.args[3], context.args[4], context.args[5]
    ...
    conn = psycopg.connect(os.environ.get('DATABASE_URL'))
    c = conn.cursor()
    c.execute('INSERT INTO schedule (group_name, day, time, subject, teacher, room) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id', (group_name, valid_days[day_short.upper()], time, subject, teacher, room))
    sid = c.fetchone()[0]
    conn.commit(); conn.close()
...
async def add_homework_command(update: Update, context):
    ...
    deadline = "Не указан"
    parts = task.split()
    if len(parts) > 1 and parts[-2].lower() == 'до': deadline = parts[-1]; task = ' '.join(parts[:-2])
    conn = psycopg.connect(os.environ.get('DATABASE_URL'))
    c = conn.cursor()
    c.execute('INSERT INTO homework (group_name, subject, task, deadline, created_at) VALUES (%s, %s, %s, %s, %s) RETURNING id', (group_name, subject, task, deadline, datetime.datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M')))
    hid = c.fetchone()[0]
    conn.commit(); conn.close()
...
async def broadcast_command(update: Update, context):
    ...
    for user_data in users:
        try:
            ...
            elif reply_msg.audio: await context.bot.send_audio(user_data[0], reply_msg.audio.file_id, caption=reply_msg.caption or "")
            elif reply_msg.voice: await context.bot.send_voice(user_data[0], reply_msg.voice.file_id)
            elif reply_msg.animation: await context.bot.send_animation(user_data[0], reply_msg.animation.file_id, caption=reply_msg.caption or "")
...
async def create_poll_command(update: Update, context):
    ...
    conn = psycopg.connect(os.environ.get('DATABASE_URL'))
    c = conn.cursor()
    c.execute('INSERT INTO polls (question, options, creator_id, created_at) VALUES (%s, %s, %s, %s) RETURNING id', (question, '|'.join(options), update.effective_user.id, datetime.datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M')))
...
async def publish_to_channel(context, anon_id):
    ...
    except Exception as e: return False, f"❌ Ошибка публикации: {e}"
