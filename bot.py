    ...
    for user_data in users:
        ...
        try:
            if reply_msg.photo:
                ...
                await context.bot.send_message(chat_id=user_id, text="📢 Объявление от администрации (неподдерживаемый формат)")
...
async def publish_to_channel(context, anon_id):
    ...
    channel_text = (
        ...
        f"{message_text}\n\n"
        f"🕐 {datetime.datetime.now(TIMEZONE).strftime('%H:%M %d.%m.%Y')}"
    )
...
# ==================== КОМАНДА /setgroup (ВЫБОР ГРУППЫ) ====================
async def setgroup_command(update: Update, context):
    user_id = update.effective_user.id
    
    if not context.args:
        await update.message.reply_text("⚠️ Пример: /setgroup 1Ю1/925o\n\nНапиши команду и название группы через пробел.")
        return
        
    group_name = context.args[0].strip()
    
    # Проверка, есть ли такая группа в нашем списке
    if group_name not in GROUPS:
        await update.message.reply_text(f"⚠️ Группа '{group_name}' не найдена!\n\nПроверь название или используй кнопку '👥 Выбрать группу' в меню.")
        return

    conn = sqlite3.connect('college_bot.db')
    c = conn.cursor()
    # Добавляем пользователя, если его вдруг нет в базе
